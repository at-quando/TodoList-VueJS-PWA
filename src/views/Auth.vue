<template>
  <section class="login">
    <div class="main-login text-center">
      ...
      <section class="mt-30">
        <router-link tag="button" :to="{ path: '/' }" class="login-btn" disabled> {{ $t('auth.google') }}</router-link>
        <router-link tag="button" :to="{ path: '/' }" class="login-btn"> {{ $t('auth.facebook') }}</router-link>
